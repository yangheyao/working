...
static int tas5717_set_master_vol(struct snd_soc_codec *codec)
{
	...
	if (pdata && pdata->custom_master_vol) {
		...
		pr_info("tas5717_volume: vol_data[0]=0x%x,vol_data[1]=0x%x\n",vol_data[0],vol_data[1]);
...
#define TAS_GPIO_PU ((GPIOW_4)+131)
#define TAS_GPIO_PU_NAME "tas_pu"
static int reset_tas5717_GPIO(struct snd_soc_codec *codec)
{
	...
	#if 1
	ret = gpio_request(TAS_GPIO_PU,TAS_GPIO_PU_NAME);
	if(ret){
		printk("tas5717_gpio pu is failed %d\n",ret);
	}
	ret = gpio_direction_output(TAS_GPIO_PU,1);
	gpio_set_value(TAS_GPIO_PU,1);
	mdelay(1000);

	printk("%s\n",__func__);
	gpio_direction_input(pdata->reset_pin);
	mdelay(1000);
...
static int phone_thread(void *codec)
{
	...
	while (1) {
		...
		else if (GPIO_LOW_LEVEL == scan_value) {
			//do {
				snd_soc_write(codec, DDX_SYS_CTL_2, 0x00);
				gpio_direction_output(pdata->scan_pin,1);
				gpio_set_value(pdata->scan_pin,1);
				//gpio_set_value(pdata->phone_pin,
				//	GPIO_LOW_LEVEL);
			//} while (
			//(GPIO_HIGH_LEVEL != gpio_get_value(pdata->phone_pin))
...
static int tas5717_init(struct snd_soc_codec *codec)
{
	...
	pr_info("channel_vol[0]::0x%x,0x%x\n",
		channel_vol[0][0], channel_vol[0][1]);
	pr_info("channel_vol[1]::0x%x,0x%x\n",
		channel_vol[1][0], channel_vol[1][1]);
	pr_info("channel_vol[2]::0x%x,0x%x\n",
