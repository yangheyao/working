...
struct aml_runtime_data {
	...
	int active;
	/*
	sometimes hwptr will not move anymore when HDMIRX plug/unplug.
	ALSA drivers will block in 10 s then timeout.
	It will block the user space function call.
	code here to check the hw ptr then add some trick here.
	*/
	/*count for every interupt callback when hw pointer is not moving*/
	unsigned int xrun_num;
	/*tricky time count,every tricky mode is 100 ms*/
	unsigned int tricky_count;
	unsigned int last_addr;/*hw addr*/
};
