				  ...
				  unsigned long delay)
{
	...
	p_aml_audio->spdif_detect_flag = -1;
...
static int aml_get_audio_spdif_status(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = audio_spdif_status;
	return 0;
}
...
static const char * const audio_spdif_status_texts[] = {"Unpluged", "Pluged"};

static const struct soc_enum audio_spdif_status_enum = SOC_ENUM_SINGLE(
	SND_SOC_NOPM, 0, ARRAY_SIZE(audio_spdif_status_texts),
	audio_spdif_status_texts);

static const struct snd_kcontrol_new spdif_controls[] = {
	   SOC_ENUM_EXT("Spdif Status",
			   audio_spdif_status_enum,
			   aml_get_audio_spdif_status,
			   NULL),
};
...
static int spdif_det_adc_value(struct aml_audio_private_data *p_aml_audio)
{
	int ret, hp_value;
	int hp_val_sum = 0;
	int loop_num = 0;
	...
	while (loop_num < 8) {
		hp_value = gpiod_get_value(p_aml_audio->spdif_det_desc);
		if (hp_value < 0) {
			pr_info("hp detect get error adc value!\n");
			return -1;
		}
...
static int aml_audio_spdif_detect(struct aml_audio_private_data *p_aml_audio)
{
	int loop_num = 0;
	int ret;
	p_aml_audio->spdif_det_status = false;

	while (loop_num < 3) {
		ret = spdif_det_adc_value(p_aml_audio);
		if (p_aml_audio->spdif_last_state != ret) {
			msleep_interruptible(50);
			if (ret < 0)
				ret = p_aml_audio->spdif_last_state;
			else
				p_aml_audio->spdif_last_state = ret;
		} else
			msleep_interruptible(50);
...
static void aml_asoc_work_func(struct work_struct *work)
{
	...
	if (p_aml_audio->hp_det_status) {
		hp_flag = aml_audio_hp_detect(p_aml_audio);

		if (p_aml_audio->detect_flag != hp_flag) {
			p_aml_audio->detect_flag = hp_flag;
	...
	if (p_aml_audio->spdif_det_status)	{
		spdif_flag = aml_audio_spdif_detect(p_aml_audio);

		if (p_aml_audio->spdif_detect_flag != spdif_flag) {
			p_aml_audio->spdif_detect_flag = spdif_flag;

			if (spdif_flag & 0x1) {
				pr_info("aml aduio spdif pluged\n");
				audio_spdif_status = 1;
			} else {
				pr_info("aml audio spdif unpluged\n");
				audio_spdif_status = 0;
			}
		}

		p_aml_audio->spdif_det_status = true;
	}
}
...
static void aml_asoc_timer_func(unsigned long data)
{
	...
	if ((p_aml_audio->hp_det_status && !p_aml_audio->suspended) ||
		(p_aml_audio->spdif_det_status && !p_aml_audio->suspended)) {
...
static int aml_asoc_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	of_property_read_u32(card->dev->of_node, "spdif_hs_switch",
		&p_aml_audio->spdif_hs_switch);

	if (p_aml_audio->spdif_hs_switch) {
		/* spdif dection gipo */
		p_aml_audio->spdif_det_desc = gpiod_get(card->dev, "spdif_det");
		if (!IS_ERR(p_aml_audio->spdif_det_desc))
			gpiod_direction_input(p_aml_audio->spdif_det_desc);
		else
			pr_err("ASoC: spdif_det-gpio failed\n");

		of_property_read_u32(card->dev->of_node,
			"spdif_det_inv",
			&p_aml_audio->spdif_det_inv);
