	...
	if (pdev->dev.of_node) {
		...
		if (of_get_property(pdev->dev.of_node,
			...
			if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXTVBB) {
				WIFI_INFO("GXTV: set pwm as 32k output");
				aml_write_cbus(0x2190, 0x16d016e);
				aml_write_cbus(0x2195, 0x16d016d);

				pwm_time_count = aml_read_cbus(0x2194);
				pwm_time_count &= ~(0xffff << 16);
				pwm_time_count |= ((3 << 16) | (2 << 24));
				aml_write_cbus(0x2194, pwm_time_count);

				pwm_misc = aml_read_cbus(0x2192);
				pwm_misc &= ~((0x7f << 8) | (3 << 4) |
					(1 << 2) | (1 << 0));
				pwm_misc |= ((1 << 25) | (1 << 15) |
					(0 << 8) | (0 << 4));
				aml_write_cbus(0x2192, (pwm_misc | (1 << 0)));
