...
int dlp_unifykey_len_check(int key_len, int len)
{
	if (key_len < len) {
...
int dlp_unifykey_header_check(unsigned char *buf,
...
#if 0
int dlp_unifykey_check(char *key_name)
{
...
int dlp_unifykey_get(char *key_name, unsigned char *buf, int *len)
{
	int key_len;
	int ret;
	...
	key_len = 0;
	/*
	ret = dlp_unifykey_check(key_name);
	if (ret < 0)
		return -1;
		*/
	ret = key_unify_size(key_name, &key_len);
...
//int dlp_unifykey_set(char *key_name, unsigned char *buf, int *len)
int dlp_unifykey_set(unsigned char *buf, int *len)
{
	int key_len;
	char *key_name;
	int ret;
	...
	*len = key_len;
	...
	ret = key_unify_write(key_name, buf, key_len);
	if (ret < 0) {
		LCDUKEYERR("%s unify read error\n", key_name);
		return -1;
	}
...
void dlp_unifykey_print(void)
{
	...
	ret = dlp_unifykey_get(key_name, buf, &key_len);
	if (ret < 0)
		return;
