...
static int set_pwmled_brightness(void)
{
	void __iomem *vaddr;
	void __iomem *vaddr1;
	void __iomem *vaddrmux;
	int pwm_misr = 0;

	vaddr = ioremap(0xc8100550, 0x4);
	vaddr1 = ioremap(0xc8100558, 0x4);
	vaddrmux = ioremap(0xc8100014, 0x4);

	pwm_misr = ((1 << 0) | (1 << 9) | (1 << 15));
	pwm_write_reg(vaddrmux, readl(vaddrmux) | (1 << 22));/*pinmux*/
	pwm_write_reg(vaddr, 0x9C3F115B);/*10% brightness*/
	/*
	*5% 0xA4EC08AE, 20% 0x8AE122B9
	*/
	pwm_write_reg(vaddr1, readl(vaddr1) | pwm_misr);/*enable*/
	pr_info("set_pwmled_brightness, 10 per\n");
