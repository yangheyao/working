...
static void bl_power_on(void)
{
	...
	case BL_CTRL_PWM_COMBO:
		...
		fan_pwm_ctrl(bconf->bl_pwm_combo0, 1);
		fan_pwm_ctrl(bconf->bl_pwm_combo1, 1);
		bl_pwm_pinmux_ctrl(bconf, 1);
		if (bconf->pwm_on_delay > 0)
			mdelay(bconf->pwm_on_delay);
		/* step 2: power on enable */
...
static void bl_power_off(void)
{
	...
	case BL_CTRL_PWM_COMBO:
		...
		if (bconf->pwm_off_delay > 0)
			mdelay(bconf->pwm_off_delay);
		fan_pwm_ctrl(bconf->bl_pwm_combo0, 0);
		fan_pwm_ctrl(bconf->bl_pwm_combo1, 0);
		bl_pwm_pinmux_ctrl(bconf, 0);
		break;
#ifdef CONFIG_AML_BL_EXTERN
	case BL_CTRL_EXTERN:
		...
		struct platform_device *pdev)
{
	...
	case BL_CTRL_PWM_COMBO:
		...
		if (ret) {
			...
			pwm_combo0->pwm_port = fan_pwm_str_to_pwm(str);
		}
		ret = of_property_read_string_index(child, "bl_pwm_combo_port",
			1, &str);
		if (ret) {
			...
			pwm_combo1->pwm_port = fan_pwm_str_to_pwm(str);
		}
		...
		/* init pwm config */
		fan_pwm_config_init(pwm_combo0);
		fan_pwm_config_init(pwm_combo1);
...
static int aml_bl_creat_class(void)
{
	...
	if (IS_ERR(fan_debug_class)) {
		BLERR("create aml_fan debug class fail\n");
		return -1;
	}
	...
	for (i = 0; i < ARRAY_SIZE(bl_debug_class_attrs); i++) {
		if (class_create_file(fan_debug_class,
			...
			BLERR("create aml_fan debug attribute %s fail\n",
...
#if 0
static int __init fan_bl_boot_para_setup(char *s)
{
...
__setup("bl_off=", fan_bl_boot_para_setup);
#endif

MODULE_DESCRIPTION("AML Fan Driver");
MODULE_LICENSE("GPL");
