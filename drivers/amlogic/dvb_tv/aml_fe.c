...
MODULE_PARM_DESC(hwid_tuner, "\n\t\t show hardware board id");
static int hwid_tuner;
module_param(hwid_tuner, int, 0644);

#define SI2151 "si2151_tuner"
#define MX1661 "mxl661_tuner"
static char gstr_tuner_name[20] = {0};

static unsigned int hwid;
static int __init hwid_setup(char *env)
{
	/*printk("[aml_fe]%s:env=%s\n", __func__, env);*/
	hwid = *env - '0';
	pr_err("[aml_fe]%s: hwid=%d. cvte={0,1}, jrx={2,4}\n", __func__, hwid);
	if (2 == hwid || 4 == hwid) {
		strcpy(gstr_tuner_name, MX1661);
		hwid_tuner = 2;
	} else {
		strcpy(gstr_tuner_name, SI2151);
		hwid_tuner = 1;
	}
	return 0;
}

__setup("hwid=", hwid_setup);
