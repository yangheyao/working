...
/* basic format*/
	...
	{HDMI_576i50, 1, 27000, 1440, 288, 288, 288, 0, 2500},
...
static int get_timing_fmt(void)
{
	...
	if (repeat_check) {
		if (freq_ref[i].repeat != 9) {
			if ((freq_ref[i].vic == HDMI_576i50) &&
				(freq_ref[i].frame_rate == 2500) &&
				(rx.pre.repeat < 2)) {
				if (log_level & PACKET_LOG) {
					rx_pr("don't check repeation\n");
				}
			} else if (rx.pre.repeat != freq_ref[i].repeat) {
				if (log_level & PACKET_LOG) {
						rx_pr("\n repeat err");
						rx_pr("%d:%d(standard)",
							rx.pre.repeat,
