...
module_param(acd_h_back, uint, 0664);
MODULE_PARM_DESC(acd_h_back, "acd_h_back");
			...
			unsigned int hcnt64)
{
	...
	/*if 0x12d is default value,0x166 bit31 enable*/
	/*when atv signal change and recover*/
	if (R_APB_REG(ACD_REG_2D) == acd_h_back)
		W_APB_BIT(ACD_REG_66, 1, AML_2DCOMB_EN_BIT,
				AML_2DCOMB_EN_WID);
}
/*
...
void tvafe_snow_config_acd(void)
{
	...
	/*0x80000f5c,bit31 enable,same with acd_table*/
	W_APB_BIT(ACD_REG_66, 1, AML_2DCOMB_EN_BIT,
				AML_2DCOMB_EN_WID);
}
/*only for pal-i*/
