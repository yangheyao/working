	...
	struct vframe_s *vf, int outfmt)
{
	...
	if (priv->src_rect.width < 0)
		input_width = vf->width;
	else if ((priv->src_rect.width > 0) && (priv->src_rect.width < vf->width - priv->src_rect.x))
		input_width = priv->src_rect.width;
	else
		input_width = vf->width - priv->src_rect.x;
#endif
	...
	if (priv->src_rect.height < 0)
		input_height = vf->height;
	else if ((priv->src_rect.height > 0) && (priv->src_rect.height < vf->height - priv->src_rect.y))
		input_height = priv->src_rect.height;
	else
		input_height = vf->height - priv->src_rect.y;
#endif
	...
	if (height_after_di == (vf->height)/2)
